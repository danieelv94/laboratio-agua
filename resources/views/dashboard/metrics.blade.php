        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Satisfacción Promedio</div>
                        <div class="text-2xl font-black text-guinda-ceaa mt-0.5">{{ number_format($overallAverage, 2) }} <span class="text-sm font-normal text-slate-400">/ 5</span></div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Encuestas Respondidas</div>
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Muestras Analizadas</div>
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Estudios Completados</div>
                        <div class="text-2xl font-black text-slate-800 mt-0.5">{{ $statusCounts['completado'] ?? 0 }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b pb-3 border-slate-100">
                        <h3 class="font-bold text-sm text-slate-800 tracking-tight font-title">Evaluación Detallada por Pregunta</h3>
                        <span class="text-[10px] bg-arena-claro/15 text-guinda-ceaa font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider">Escala 1 al 5</span>
                    </div>
                    <div class="h-80 relative flex items-center justify-center">
                        @if($totalSurveys > 0)
                            <canvas id="surveyQuestionsChart"></canvas>
                        @else
                            <p class="text-xs text-slate-400 italic">No hay suficientes datos de encuestas para mostrar este gráfico.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b pb-3 border-slate-100">
                        <h3 class="font-bold text-sm text-slate-800 tracking-tight font-title">Distribución de Tipos de Muestra</h3>
                        <span class="text-[10px] bg-slate-50 text-slate-500 font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider">Muestras Activas</span>
                    </div>
                    <div class="h-80 relative flex items-center justify-center">
                        @if(count($sampleTypeCounts) > 0)
                            <canvas id="sampleTypesChart"></canvas>
                        @else
                            <p class="text-xs text-slate-400 italic">No hay suficientes datos de muestras para mostrar este gráfico.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b pb-3 border-slate-100">
                        <h3 class="font-bold text-sm text-slate-800 tracking-tight font-title">Normativas Solicitadas</h3>
                        <span class="text-[10px] bg-slate-50 text-slate-500 font-bold px-2 py-0.5 rounded-full uppercase">Frecuencia</span>
                    </div>
                    <div class="h-80 relative flex items-center justify-center">
                        @if(count($normativaCounts) > 0)
                            <canvas id="normativesChart"></canvas>
                        @else
                            <p class="text-xs text-slate-400 italic">No hay suficientes datos de normativas para mostrar este gráfico.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b pb-3 border-slate-100">
                        <h3 class="font-bold text-sm text-slate-800 tracking-tight font-title">Estudios Completados por Mes</h3>
                        <span class="text-[10px] bg-emerald-50 text-emerald-600 font-bold px-2 py-0.5 rounded-full uppercase">Histórico</span>
                    </div>
                    <div class="h-80 relative flex items-center justify-center">
                        @if(count($monthlyCounts) > 0)
                            <canvas id="monthlyChart"></canvas>
                        @else
                            <p class="text-xs text-slate-400 italic">Aún no hay estudios completados registrados en el sistema.</p>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
                <h3 class="font-bold text-sm text-slate-800 tracking-tight border-b pb-3 border-slate-100 mb-4 font-title">Referencia de Preguntas Evaluadas</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 text-xs text-slate-600">
                    ...
                    <div><span class="font-bold text-guinda-ceaa">3.</span> Cumplimiento de requerimientos técnicos</div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="font-bold text-sm text-slate-800 tracking-tight font-title">Comentarios y Recomendaciones Recientes</h3>
                        ...
                        <div class="p-6 hover:bg-slate-50/50 transition space-y-3">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('dashboard.solicitud', $feedback->id) }}" class="font-bold text-guinda-ceaa hover:underline tracking-wide">
                                ...
                                <div class="flex items-center space-x-3 text-slate-400">
                                    ...
                                    <div class="flex items-center space-x-1 font-bold text-guinda-ceaa">
                                        <svg class="w-4 h-4 text-dorado-ocre fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                                @if($feedback->encuesta_mejoras)
                                    <div class="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 font-title">Recomendación para mejorar:</span>
                                        <p class="text-slate-600 italic">"{{ $feedback->encuesta_mejoras }}"</p>
                                    </div>
                                @endif
                                @if($feedback->encuesta_comentarios)
                                    <div class="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 font-title">Comentario adicional:</span>
